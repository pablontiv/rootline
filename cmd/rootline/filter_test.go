...
func TestFilterRecords_MatchSubset(t *testing.T) {
	...
	filtered, err := filterRecords(records, []string{"estado == 'Pending'"})
	if err != nil {
...
func TestFilterRecords_NoMatch(t *testing.T) {
	...
	filtered, err := filterRecords(records, []string{"estado == 'Completed'"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(filtered) != 0 {
...
func TestFilterRecords_InvalidExpr(t *testing.T) {
	...
	_, err := filterRecords(records, []string{"== bad syntax"})
	if err == nil {
...
func TestFilterRecords_EmptyWheres(t *testing.T) {
	...
	filtered, err := filterRecords(records, nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(filtered) != 2 {
	...
	filtered, err = filterRecords(records, []string{})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(filtered) != 2 {
...
func TestFilterRecords_MultipleWheres(t *testing.T) {
	...
	filtered, err := filterRecords(records, []string{"estado == 'Pending'", "tipo == 'test'"})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(filtered) != 1 {
