...
- `.stem` schema fields set to `null` in a child file are now genuinely removed from the effective schema, implementing the documented removal syntax that was promised but did not work. A `null` value is deleted at the merge layer and skipped by stem-health checks (`field-declaration`, `type-consistency`, `monotonic-violations`).
