...
**Despues**: Archivos Go se llaman `link_validation.go`, `back_references.go`. Task files usan slugs descriptivos. Struct Inference no tiene campo `Category` (el `Type` string ya identifica la inferencia). Cero referencias a "Cat N" en código Go.
...
- [ ] `rootline validate --all docs/epics/E13-inference-engine/F02-category-expansion/` pasa
