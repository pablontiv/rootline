---
name: rootline
description: Use when working with Markdown records governed by .stem schemas or when the user asks to validate, fix, query, inspect, scaffold, mutate, analyze, apply, graph, trace, or serve Rootline data, even if they do not name Rootline.
---

# Rootline CLI Operations

Rootline is the primary interface for `.stem`-governed Markdown data. Use `rootline` commands before manual file inspection.

## Source of Truth

Before relying on a command detail, verify it locally:
...
4. **Mutations**: run the command-specific preview first. Apply changes only when the user explicitly requested a write or approves the preview.
5. **Verify writes**: after any mutation, run the smallest matching `rootline validate` command and compare the changes via git diff when files changed.
