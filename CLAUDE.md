...
- `cmd/rootline/` — CLI entry point. Each subcommand is a separate file (validate.go, query.go, describe.go, init.go, new.go, fix.go, tree.go, stats.go, hooks.go, completion.go, migrate.go, graph.go, explain.go, serve.go, analyze.go, apply.go). Helpers: table.go, filter.go (output formatting and record filtering). Uses cobra with global flags `--output json|table`, `--field` (dot-path extraction). `analyze` supports `--incremental` to filter inferences covered by existing `.stem`. `apply` reads analyze report and modifies `.stem` files.
