...
- `internal/fix/` — Fix application: rewrites frontmatter based on proposals. Uses `fuzzy.Match` for enum correction. Type-representation repairs use an isolated guard: when `from_representation` is present, the current scalar lexeme and representation must exactly match the report and unknown markers fail closed; changed lexemes, changed representations, already-quoted typed repairs, missing metadata, and missing fields are rejected without writing. `repair.go` exports `ApplyRepair(proposals, dryRun, root)` for data-only bulk repair: accepts repair-surface proposals only, never writes `.stem` files, supports dry-run, post-validates modified files. `ApplySchemaProposals(ctx, report, root)` applies schema-surface proposals (extend_enum) from a separated report; counterpart to `ApplyProposals` for the schema path.
...
- `internal/skilldist/` — Product-owned agent skill distribution engine used by `rootline skill install|status|uninstall|restore`. It resolves the canonical source from a primary Rootline checkout, inventories `~/.claude/skills/rootline` and `~/.agents/skills/rootline`, builds approval-bound plans, publishes managed symlinks, stores `receipts.jsonl` and backups under the user state root, and refuses stale approvals, linked worktree sources, unsupported destination types, and symlink permission failures.
...
- `internal/proposal/` — Proposal analysis engine: detects fixable validation errors and generates typed proposals (extend_enum, correct_value, add_field, migrate_value, etc.). Safe type-representation repairs reuse `correct_value` with optional `from_representation` for timestamp/boolean/integer-to-string quoting; unsupported type mismatches are emitted as `type_findings` instead of coercions. `surface.go` defines `ProposalSurface` enum (schema/repair/bootstrap/migration/diagnostic/requires_agent) and `Surface()` classifier so engines can distinguish schema-mutating from document-repair proposals without inspecting command context.
