...
func TestHierarchyResult_ToLevelsMap(t *testing.T) {
	root := "/tmp/epics"
	records := makeHierarchyRecords(root, []string{
		"E01-a/README.md",
		"E02-b/README.md",
	...
	levelsMap := result.ToLevelsMap()
	if levelsMap == nil {
		t.Fatal("expected non-nil levels map")
	}

	// Should have 4 levels: epic, feature, story, task
	if len(levelsMap) != 4 {
		t.Fatalf("expected 4 levels, got %d", len(levelsMap))
	}
	...
	expectations := map[string]struct {
		...
		"task":    {match: "T???-*", children: nil},
	}

	for name, exp := range expectations {
		level, ok := levelsMap[name]
		if !ok {
			t.Errorf("missing level %q", name)
			continue
		}
		if level.Match != exp.match {
			t.Errorf("level %q: match=%q, want %q", name, level.Match, exp.match)
		}
		if len(exp.children) == 0 {
			if len(level.Children) != 0 {
				t.Errorf("level %q: expected no children, got %v", name, level.Children)
			}
		} else {
			if len(level.Children) != len(exp.children) {
				t.Errorf("level %q: children=%v, want %v", name, level.Children, exp.children)
			} else {
				for j, c := range exp.children {
					if level.Children[j] != c {
						t.Errorf("level %q: children[%d]=%q, want %q", name, j, level.Children[j], c)
					}
				}
			}
		}
	...
	// Task level should have per-level schema fields (tipo)
	taskLevel := levelsMap["task"]
	if _, ok := taskLevel.Schema["tipo"]; !ok {
		t.Error("expected 'tipo' in task level schema")
	}

	// Each level should have id sequence
	for name, level := range levelsMap {
		if _, ok := level.Schema["id"]; !ok {
			t.Errorf("level %q: expected 'id' in schema", name)
		}
	}
}

func TestHierarchyResult_ToLevelsMap_NotDetected(t *testing.T) {
	result := &HierarchyResult{Detected: false}
	levelsMap := result.ToLevelsMap()
	if levelsMap != nil {
		t.Errorf("expected nil levels map for undetected hierarchy, got %v", levelsMap)
	}
}
