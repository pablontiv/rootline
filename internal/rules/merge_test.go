...
func TestMergeLevelsMap(t *testing.T) {
	entries := []StemEntry{
		stemEntry("parent/.stem", &StemFile{
	...
	if len(result.Levels) != 3 {
		t.Fatalf("levels count = %d, want 3", len(result.Levels))
	}
	if result.Levels["epic"] == nil || result.Levels["epic"].Match != "E*" {
		t.Error("epic level not preserved from parent")
	}
	if result.Levels["feature"] == nil || result.Levels["feature"].Match != "F*" {
		t.Error("feature level not preserved from parent")
	}
	if result.Levels["story"] == nil || result.Levels["story"].Match != "S*" {
		t.Error("story level not added from child")
	}
}

func TestMergeLevelsOverride(t *testing.T) {
	entries := []StemEntry{
		stemEntry("parent/.stem", &StemFile{
			Levels: map[string]*HierarchyLevel{
				"epic": {Match: "E*", Children: []string{"feature"},
		...
		stemEntry("child/.stem", &StemFile{
			Levels: map[string]*HierarchyLevel{
				"epic": {Match: "E*", Children: []string{"feature"},
					Schema: map[string]SchemaField{"id": {Type: "sequence", Prefix: "E", Digits: 3}}},
			},
		}),
	}

	result := MergeStemFiles(entries)
	epic := result.Levels["epic"]
	if epic == nil {
		t.Fatal("epic level missing")
	}
	if epic.Schema["id"].Digits != 3 {
		t.Errorf("epic.Schema[id].Digits = %d, want 3 (child override)", epic.Schema["id"].Digits)
	}
}

func TestMergeLevelsNilChild(t *testing.T) {
	entries := []StemEntry{
		stemEntry("parent/.stem", &StemFile{
			Levels: map[string]*HierarchyLevel{
				"epic": {Match: "E*"},
		...
		stemEntry("child/.stem", &StemFile{
			Schema: map[string]SchemaField{"extra": {Type: "string"}},
		}),
	}

	result := MergeStemFiles(entries)
	if len(result.Levels) != 1 {
		t.Fatalf("levels count = %d, want 1 (preserved from parent)", len(result.Levels))
	}
	if result.Levels["epic"] == nil {
		t.Error("epic level not preserved")
	}
}

func TestMergeLevelsNilParent(t *testing.T) {
	entries := []StemEntry{
		stemEntry("parent/.stem", &StemFile{
			Version: 1,
		}),
	...
	result := MergeStemFiles(entries)
	if len(result.Levels) != 1 {
		t.Fatalf("levels count = %d, want 1", len(result.Levels))
	}
	if result.Levels["task"] == nil {
		t.Error("task level not added from child")
	}
}
